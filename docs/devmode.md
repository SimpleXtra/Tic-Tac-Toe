...
### Training with `autoplay()`
`autoplay()` is commonly used to speed up the learning process of the computer. With `autoplay()`, you don't have to play with the computer for hundreds of thousands of games. Instead, the function will play for you for an arbitrary number of games. Every time it's your turn, `autoplay()` will fill your move automatically so the computer can quickly provide a response.

The `autoplay()` function takes two parameters:
```
autoplay(iterations, showProgress)
```
...
Past moves:  X7O6X1O3X4
You Win!
Score: 1 VS 0
##################################################
Autoplay finished after 1 games in 0.06759762763977051 seconds.
```
#### **showProgress = False**
Input:
```
Your move: autoplay(100, False)
```
Output:
```
...
Autoplay finished after 100 games in 0.01999521255493164 seconds.
```
...
### `comp.cells`, the decision tree
`comp.cells` is a dictionary that stores the explored game tree. When the application starts, `comp.cells` is just an empty dictionary. As you play (or `autoplay()`), the computer stores information about every game. Then, the computer will try to eliminate the losing moves so it can play optimally. This process is done automaticallyevery time a game ends.

You can access `comp.cells` to examine the decision tree which the computer generated. For example:
```
Your move: print(comp.cells)
...
```
Your move: print(len(comp.cells))
263
```
